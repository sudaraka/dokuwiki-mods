...
function p_xhtml_cached_geshi($code, $language, $wrapper='pre') {
	...
	if($ctime && !$_REQUEST['purge'] &&
		...
		if(in_array($language, array('cli'))) $geshi->enable_line_numbers(GESHI_NO_LINE_NUMBERS);
		...
		$highlighted_code = $geshi->parse_code() . '<style type="text/css">' . $geshi->get_stylesheet() . '</style>';
		io_saveFile($cache,$highlighted_code);
	}

	// add a wrapper element if required
	//if ($wrapper)
		//$highlighted_code = "<$wrapper class=\"code $language\">$highlighted_code</$wrapper>";
